...
class ETAPredictor:
    ...
    # Maps a model_family name to the (ftl_key, cart_key) entries expected
    # in the loaded models dict. "histgbm" is the long-standing default and
    # is always required; "xgboost"/"lightgbm" are optional — only usable
    # once models/boosted_models.pkl has been merged in (see load_boosted).
    _MODEL_FAMILY_KEYS = {
        "histgbm":  ("hgbm_ftl", "hgbm_cart"),
        "xgboost":  ("xgb_ftl",  "xgb_cart"),
        "lightgbm": ("lgb_ftl",  "lgb_cart"),
    }

    @classmethod
    def load_boosted(cls, models_dir: str, reports_dir: str) -> "ETAPredictor":
        """
        Like `load`, but also merges in models/boosted_models.pkl (produced by
        `python3 -m src.training.boosted_models`) if present, so that
        model_family="xgboost" / "lightgbm" become available in `predict()`.

        Works even if models/improved_models.pkl (the HistGBM artefact from
        advanced_models.py) hasn't been generated yet — in that case only
        the boosted (XGBoost/LightGBM) model_family options are usable.
        """
        boosted_path  = os.path.join(models_dir, "boosted_models.pkl")
        improved_path = os.path.join(models_dir, "improved_models.pkl")
        ...
        if not models:
            raise FileNotFoundError(
                f"Neither {improved_path} nor {boosted_path} exists. Run "
                f"`python3 -m src.training.advanced_models` and/or "
                f"`python3 -m src.training.boosted_models` first."
            )
    ...
    def predict(
        ...
        Args:
            ...
            model_family: "histgbm" (default), "xgboost", or "lightgbm".
                The latter two require the predictor to have been loaded via
                `ETAPredictor.load_boosted(...)` (see src/training/boosted_models.py).
        ...
        if model_family not in self._MODEL_FAMILY_KEYS:
            raise ValueError(
                f"model_family must be one of {list(self._MODEL_FAMILY_KEYS)}, got '{model_family}'"
            )

        ftl_key, cart_key = self._MODEL_FAMILY_KEYS[model_family]
        model_key = ftl_key if route_type == "FTL" else cart_key
        if model_key not in self.models:
            raise ValueError(
                f"model_family='{model_family}' requested but '{model_key}' isn't loaded. "
                f"Use ETAPredictor.load_boosted(...) to include XGBoost/LightGBM models "
                f"(run `python3 -m src.training.boosted_models` first)."
            )
        model = self.models[model_key]
        family_label = {"histgbm": "HistGBM", "xgboost": "XGBoost", "lightgbm": "LightGBM"}[model_family]
        model_name = f"{route_type}-specific {family_label}"
        ...
        return ETAPrediction(
            ...
            confidence_note=f"{model_name} with 49 graph-enhanced features",
        )
