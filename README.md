...
[![Tests](https://img.shields.io/badge/Tests-26%20passing-brightgreen)](#-usage)
...
- **Full-scale boosted benchmarking** — re-run `boosted_models.py` on the full 144K-row dataset (current numbers are a 5k-row smoke test) to confirm the projected 3–5% RMSE lift from XGBoost/LightGBM.
- **Real-time hub dwell time** — replace the static risk score with a live feed from facility sorting/loading systems, where available.
- **Corridor-level model** — extend the per-route-type split (FTL vs Carting) to a per-corridor model for the highest-volume lanes.
- **REST API deployment** — wrap `ETAPredictor` in a FastAPI service; see [`docs/deployment.md`](docs/deployment.md) for the planned approach.
- **Automated retraining pipeline** — scheduled retraining as new shipment data lands, with drift monitoring on RMSE and ±15% accuracy.
...
2. Make your changes, keeping new logic covered by tests under `tests/`.
3. Run the test suite (`python3 -m unittest discover tests/ -v`) and confirm nothing regresses.
4. Open a pull request describing the change and, where relevant, its effect on the benchmark metrics.

For larger changes (new model families, new graph variants, dashboard pages), please open an issue first to discuss the approach — see [`docs/project_decisions.md`](docs/project_decisions.md) for the reasoning behind existing design choices.
